...
**Tower vs mini:** Polygon and some secrets may live on the **mini** while bots run on **tower**. ModelRouter on the mini is the **single front door**; tower clients use the gateway URL from `config/hosts.yaml` (LAN or Tailscale). Keys stay on the mini — tower doesn’t duplicate `.env` files.
