...
usage() {
  echo "Usage: connect-anthropic.sh [--no-push] [--no-restart] [--stash-alt]"
  echo "  Paste Anthropic API key (sk-ant-…), save to .env, push to mini, restart gateway."
  echo "  --stash-alt  move previous ANTHROPIC_API_KEY → ANTHROPIC_API_KEY__ALT_1 when replacing"
...
save_key() {
  ANTHROPIC_API_KEY="$1" STASH_ALT="$STASH_ALT" PYTHONPATH="$ROOT" "$ROOT/.venv/bin/python" -c "
import os
...
if env_path.exists():
    for line in env_path.read_text().splitlines():
        if line.startswith('ANTHROPIC_API_KEY=') and not line.startswith('ANTHROPIC_API_KEY__'):
            old = line.split('=', 1)[1].strip()
            break

if stash and old and old != key:
    update_env_file(env_path, 'ANTHROPIC_API_KEY__ALT_1', old)
    print('  ok stashed previous key → ANTHROPIC_API_KEY__ALT_1')

update_env_file(env_path, 'ANTHROPIC_API_KEY', key)
...
if [[ "$NO_PUSH" -eq 0 && "$PUSH" -eq 1 ]]; then
  ...
  PUSH_KEYS=(ANTHROPIC_API_KEY)
  if grep -q '^ANTHROPIC_API_KEY__ALT_1=.' "$ENV_FILE" 2>/dev/null; then
    PUSH_KEYS+=(ANTHROPIC_API_KEY__ALT_1)
  fi
