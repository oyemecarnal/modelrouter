...
usage() {
  echo "Usage: connect-openai.sh [--no-push] [--no-restart]"
...
save_key() {
  OPENAI_API_KEY="$1" PYTHONPATH="$ROOT" "$ROOT/.venv/bin/python" -c "
import os
...
update_env_file(Path('''$ENV_FILE'''), 'OPENAI_API_KEY', key)
...
if [[ "$NO_PUSH" -eq 0 && "$PUSH" -eq 1 ]]; then
  ...
  "$ROOT/scripts/push-env-to-mini.sh" OPENAI_API_KEY
else
