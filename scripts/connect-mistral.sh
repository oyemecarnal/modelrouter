...
usage() {
  echo "Usage: connect-mistral.sh [--no-push] [--no-restart]"
...
save_key() {
  MISTRAL_API_KEY="$1" PYTHONPATH="$ROOT" "$ROOT/.venv/bin/python" -c "
import os
...
update_env_file(Path('''$ENV_FILE'''), 'MISTRAL_API_KEY', key)
...
if [[ "$NO_PUSH" -eq 0 && "$PUSH" -eq 1 ]]; then
  ...
  "$ROOT/scripts/push-env-to-mini.sh" MISTRAL_API_KEY
else
